...
import { verified } from '../../redux/slice/OtpSlice';
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (response.data.message == 'User registered, OTP sent') {
        dispatch(verified('otpVerification'))
        setEmail(email);
        setIsOtpSend(true);
        navigate('/signup_otp', { state: { email } });
      }
    } catch (err) {
          ...
          <div className="w-1/2 p-8">
            ...
            <form className="space-y-4" onSubmit={handleSubmit(onSubmit)}>
              <div>
                ...
                />
                {errors.fullName ? <p className="text-red-600 text-sm mt-1">{errors.fullName.message}</p> : <p> </p>}
              </div>
              <div>
                <label className="block text-sm font-medium text-zinc-700">
                  Email
                ...
                <input
                  ...
                  placeholder="Email"
                />
                {errors.email ? <p className="text-red-600 text-sm mt-1">{errors.email.message}</p> : <p> </p>}
              ...
              <div>
                ...
                />
                {errors.password ? <p className="text-red-600 text-sm mt-1">{errors.password.message}</p> : <p> </p>}
              </div>
              <div>
